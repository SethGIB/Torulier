...
void InstancedTeapotsApp::setup()
{
	...
	gl::VboMeshRef mesh = gl::VboMesh::create(geom::Sphere().subdivisions(16).radius(1.0f));
	...
	for (size_t potX = 0; potX < NUM_INSTANCES_X; ++potX) {
		for (size_t potY = 0; potY < NUM_INSTANCES_Y; ++potY) {
			...
			mPositions.push_back(vec3(instanceX * vec3(DRAW_SCALE, 0, 0) + instanceY * vec3(0, 0, DRAW_SCALE)));
		}
	}

	// create the VBO which will contain per-instance (rather than per-vertex) data
	mInstanceDataVbo = gl::Vbo::create(GL_ARRAY_BUFFER, mPositions.size() * sizeof(vec3), mPositions.data(), GL_DYNAMIC_DRAW);
...
void InstancedTeapotsApp::draw()
{
	...
	mGlsl->uniform("uColor", ColorAf(1.f, .5f, 0.f, 1.f));
